...
![HostPilot UI](docs/screenshot.svg)
...
```powershell
git clone https://github.com/shifty81/HostPilotLatest.git
cd HostPilotLatest

# Build (requires .NET 9 SDK)
dotnet build HostPilot.sln

# Run
dotnet run --project HostPilot
```

### Option B — Open in Visual Studio

1. Install **Visual Studio 2022** (Community edition is free).
2. Open `HostPilot.sln` — all three projects load automatically.
3. Set **HostPilot** as the startup project and press **F5**.
...
```
HostPilotLatest/
├── HostPilot/           # WPF UI (MainWindow, WorkshopBrowserWindow)
│   └── Themes/DarkTheme.xaml  # Dark colour-palette resource dictionary
├── HostPilot.Core/      # Business logic — no UI dependencies
...
├── HostPilot.Tests/     # xUnit test suite (ServerConfig, ServerManager)
├── Scripts/
...
4. Run the test suite to verify nothing is broken (`dotnet test HostPilot.sln`)
